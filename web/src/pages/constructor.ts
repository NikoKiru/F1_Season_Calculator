/**
 * Constructor detail page: stat tiles are SSR; the position-bar and
 * season-length line charts fetch + render client-side. Mirrors driver.ts.
 */
...
import type { ConstructorStats } from "../lib/types";
...
async function hydrateCharts(payload: PagePayload): Promise<void> {
  ...
  let stats: ConstructorStats;
  try {
    stats = await apiGet<ConstructorStats>(
      `/api/constructors/${encodeURIComponent(payload.slug)}/stats`,
